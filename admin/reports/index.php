<?php
$from = isset($_GET['from']) ? $_GET['from'] : date('d-m-Y', strtotime(date("d-m-Y") . '-1 week'));
...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title">Báo cáo đặt chỗ</h3>
    </div>
    <div class="card-body">
        <fieldset>
            <legend class="text-muted">Lọc</legend>
            <form action="" id="filter-report">
                <div class="row align-items-end">
                    <div class="form-group col-md-3">
                        <small class="text-muted mx-2">Từ ngày</small>
                        <input type="date" name="from" value="<?= $from ?>"
                            class="form-control form-control-sm rounded-0" required>
                    </div>
                    <div class="form-group col-md-3">
                        <small class="text-muted mx-2">Ngày tới</small>
                        <input type="date" name="to" value="<?= $to ?>" class="form-control form-control-sm rounded-0"
                            required>
                    </div>
                    <div class="form-group col-md-3">
                        <button class="btn btn-primary btn-flat btn-sm"><i class="fa fa-filter"></i> Lọc</button>
                        <button class="btn btn-success btn-flat btn-sm" type="button" id="print"><i
        ...
        <div class="container-fluid" id="outprint">
            ...
            <div id="show-print">
                <div class="w-100 position-relative">
                    <img src="<?= validate_image($_settings->info('logo')) ?>" id="logo" alt="Logo"
                        class="img-circle position-absolute border">
                    <h3 class="m-0 text-center">
                        <?= $_settings->info('name') ?>
                    </h3>
                    <h4 class="text-center"><b>Danh sách đặt chỗ</b></h4>
                    <center><small>
                            <?php
                            if ($from == $to) {
                ...
                <tbody>
                    ...
                    while ($row = $qry->fetch_assoc()):
                        ...
                        <tr>
                            ...
                            <td class="px-0">
                                <div class="border-bottom px-2"><span class="text-muted"><i class="fa fa-calendar"></i>
                                        Vào:
                                    </span>
                                    <?= $row['check_in'] ?>
                                </div>
                                <div class="px-2"><span class="text-muted"><i class="fa fa-calendar"></i> Ra: </span>
                                    <?= $row['check_out'] ?>
                                </div>
                            </td>
                            <td class="px-0">
                                <div class="border-bottom px-2"><span class="text-muted">Phòng: </span>
                                    <?= $row['room_name'] ?>
                                </div>
                                <div class="px-2"><span class="text-muted">Kiểu Phòng: </span>
                                    <?= $row['room_type'] ?>
                                </div>
                            </td>
                            <td class="text-center">
                                ...
                                switch ($row['status']) {
                                    case 0:
                                        echo '<span class="rounded-pill badge badge-secondary col-6">Chờ giải quyết</span>';
                                        break;
                                    case 1:
