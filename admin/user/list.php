										...
										<?php if ($row['status'] != 1): ?>
											<a class="dropdown-item verify_user" href="javascript:void(0)"
												data-id="<?= $row['id'] ?>" data-name="<?= $row['username'] ?>"><span
													class="fa fa-check text-primary"></span> Xác minh</a>
											<div class="dropdown-divider"></div>
										<?php endif; ?>
...
<script>
	$(document).ready(function () {
		$('.delete_data').click(function () {
			_conf("Bạn có chắc chắn xóa Người dùng này vĩnh viễn?", "delete_user", [$(this).attr('data-id')])
		})
		$('.table td,.table th').addClass('py-1 px-2 align-middle')
		$('.table').dataTable();
		$('.verify_user').click(function () {
			_conf("Bạn có chắc chắn xác minh <b>" + $(this).attr('data-name') + "<b/>?", "verify_user", [$(this).attr('data-id')])
		})
	})
