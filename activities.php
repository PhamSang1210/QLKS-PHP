                        ...
                        while ($row = $activitys->fetch_assoc()):
                            ...
                            <div class="col">
                                <a href="javascript:void(0)" data-id="<?= $row['id'] ?>"
                                    class="activity-item card rounded-0 shadow flex-row text-decoration-none text-dark p-0">
                                    <div class="col-auto p-0">
                                        <div class="activity-holder overflow-hidden">
                                            <img src="<?= validate_image($row['image_path']) ?>"
                                                class="img-top rounded-0 activity-img" alt="<?= $row['name'] ?> Image">
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <div
                                            class="container-fluid p-1 m-0 h-100 d-flex flex-column justify-content-center">
...
<script>
    $(function () {
        ...
        $('.activity-item').click(function () {
            uni_modal("Chi tiết hoạt động", 'view_activity.php?id=' + $(this).attr('data-id'), 'mid-large')
        })
    })
</script>